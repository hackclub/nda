...
The agreement in `app/models/nda_document.rb` is based on the Hack Club Contributor NDA last revised 2025-04-01: its original wording, numbering and sub-clause lettering were verified word for word against a signed PDF. The source renumbered its sections at some point and left five cross-references pointing at the wrong ones; those are corrected here and flagged with a comment at each site. The 2026-09-17 revision adds signer context about confidential access and volunteer, employee, or contractor status. `LEGACY_TEXT` preserves the prior document for import verification. Do not reintroduce the incorrect cross-references, and do not make further edits without an explicit request; changing the legal text requires bumping `NdaDocument::VERSION` (it is immutable once signatures exist).
